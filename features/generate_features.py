...
def analyze_cif_properties_with_embedding(cif_file, modelpath, predict_script, atom_init_json_path):
    ...
    # 임시 디렉토리 생성 및 CIF 파일 복사
    tmp_dir = 'temp_cif_for_embedding'
    os.makedirs(tmp_dir, exist_ok=True)
    cif_basename = os.path.basename(cif_file)
    shutil.copy(cif_file, tmp_dir)
    shutil.copy(atom_init_json_path, tmp_dir)
    ...
    # 임베딩 결과 CSV 로드
    embedding_csv = 'test_results.csv'
    ...
    # 임시 파일 및 폴더 안전하게 삭제 (중요!)
    shutil.rmtree(tmp_dir)
    os.remove(embedding_csv)

    return extended_results

#cif_file = 'root_dir_novamag/Fe8Ta4_#194_1N.cif'
cif_file = 'root_dir_Eh0.1_FM_ICSD/1196262.cif'
...
final_vector = analyze_cif_properties_with_embedding(cif_file, modelpath, predict_script, atom_init_json_path)
#%% ML input feature 생성
...
if __name__ == "__main__":
    input_csv = input("입력 CSV 파일 경로를 입력하세요: ").strip()
    output_csv = input("출력 CSV 파일 경로를 입력하세요: ").strip()
    mode = input("데이터 유형을 선택하세요 (l: labeled, u: unlabeled): ").strip().lower()
    if mode not in ['l', 'u']:
        raise ValueError("❌ 모드는 'l' 또는 'u' 중 하나여야 합니다.")

    root_dir_1 = 'root_dir_Eh0.1_FM_ICSD'
    root_dir_2 = 'root_dir_novamag'
    root_dir_3 = 'C:/Users/hjkim/Downloads/mattergen/results/hard_magnets_symmetrized_final/generated_crystals_cif'
